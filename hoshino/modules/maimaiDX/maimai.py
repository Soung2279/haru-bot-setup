...
@sv.on_fullmatch(["帮助maimaiDX查询", "帮助maimaiDX", "帮助DX查询"])
async def bangzhu_maimai(bot, ev):
...
@sv.on_fullmatch(['今日mai', '今日舞萌'])
async def day_mai(bot, ev:CQEvent):
    botname = hoshino.config.NICKNAME
    ...
    for i in range(11):
        if wm_value[i] == 3:
            ...
            msg += f'忌 {wm_list[i]}\n'
    msg += f'{botname[0]}提醒您：打机时不要大力拍打或滑动哦\n今日推荐歌曲：'
