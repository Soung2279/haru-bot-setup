...
import asyncio
from os import path
from hoshino import aiorequests, priv, config
import time
import nonebot
...
@sv.on_fullmatch(["帮助B站动态", "帮助bilidynamicpush", "bilidynamicpush帮助"])
async def bangzhu_bilidynamic(bot, ev):
    gid = ev.group_id
...
七海Nana7mi-434334701
AIChannel官方-1473830
AIChannel中国绊爱-484322035
……
'''.strip()

@sv.on_fullmatch(["常用UID一览"])
async def menu_uid_bilipush(bot, ev):
    await bot.send(ev, '请选择需要查看的UID列表，当前可提供：常用手游UID一览、常用VTB/VUP一览')

@sv.on_fullmatch(["常用手游UID一览"])
async def list_uid_game_bilipush(bot, ev):
    gid = ev.group_id
    if forward_msg_exchange == 1:
        msg = common_GAME_UID_LIST  #转发文本
        data = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",  #bot转发消息中的名称
