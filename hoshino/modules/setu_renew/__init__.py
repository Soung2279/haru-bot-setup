...
sv_help = '''
- [来n张xxx涩图]  来n张xxx的涩图(不指定数量与关键字发送一张随机涩图)
- [涩图]  发送一张随机涩图
- [提取图片+pid]  获取指定pid的p站图片，没有时发送链接
- [本日涩图排行榜+页数]  获取第几页p站排行榜
- [看涩图+名次]  获取p站排行榜第n名
- [看涩图n p]  获取p站排行榜从n名到p名]
- [查询涩图配置]  查看维护设置的配置
'''.strip()
...
@sv.on_fullmatch(["帮助在线涩图"])
async def bangzhu_setu_renew(bot, ev):
...
- [setu缓存]
- [setu仓库]
- [setu 黑/白名单 新增/删除 gid]  #一次只能提供一个
======================
acggov :开 / 关 是否开启acggov模块
lolicon : 开 / 关 是否开启lolicon模块
lolicon_r18 : 开 / 关 是否开启lolicon_r18模块
撤回 : ` 发出的图片在n秒后撤回, 设置为0表示不撤回
'''.strip()

@sv.on_fullmatch(["查询涩图配置"])
async def manual_setu_renew(bot, ev):
    await bot.send(ev, MANUAL_MENU_HELP, at_sender=True)
...
config_default = {
	...
	"default": {
		"withdraw": 30,  # 撤回时间，单位秒
		"lolicon": True,  # lolicon模块开关
		"lolicon_r18": False,  # lolicon_r18模块开关
		"acggov": True  # acggov模块开关
	},
	"lolicon": {
		"mode": 2,  # 0禁用 1无缓存 2有缓存在线 3有缓存离线
		"apikey": ["710713975eddf763c285f5"],  # lolicon API，可多个
		"r18": False,  # R18图开关
	...
	"acggov": {
		"mode": 2,  # 0禁用 1无缓存 2有缓存在线 3有缓存离线
		"apikey": "eyJhbGciOiJIUzI1NiJ9..3k1A89jKX8jiF2d80yP2L5EHC-tP2xLFLaqj10LxleQ",  # acggov API
