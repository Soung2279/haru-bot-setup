...
RARE_PROBABILITY = 0.25         # 戳一戳获得稀有卡片的概率
SUPER_RARE_PROBABILITY = 0.10   # 戳一戳获得超稀有卡片的概率
...
RESET_HOUR = 1                  # 每日戳一戳、赠送等指令使用次数的重置时间，0代表凌晨0点，1代表凌晨1点，以此类推
...
poke_setup_help = '''
每一戳的卡片掉落几率{POKE_GET_CARDS}
机器人每天掉落卡片的次数{POKE_DAILY_LIMIT}
戳一戳获得稀有卡片的概率{POKE_DAILY_LIMIT}
戳一戳获得超稀有卡片的概率{POKE_DAILY_LIMIT}
换卡请求的等待时间{POKE_DAILY_LIMIT}
每人每天最多接受几次赠卡{POKE_DAILY_LIMIT}
每日戳一戳、赠送等指令使用次数的重置时间{POKE_DAILY_LIMIT}:00
'''.strip()

@sv.on_fullmatch(('查看戳一戳配置'))
async def setup_poke(bot, ev):
    if forward_msg_exchange == 1:
        data_all = []
        msg1 = f'每一戳的卡片掉落几率{POKE_GET_CARDS}'
        data1 = {
            "type": "node",
            "data": {
        ...
        msg2 = f'机器人每天掉落卡片的次数{POKE_DAILY_LIMIT}'
        data2 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        msg3 = f'戳一戳获得稀有卡片的概率{POKE_DAILY_LIMIT}'
        data3 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        msg4 = f'戳一戳获得超稀有卡片的概率{POKE_DAILY_LIMIT}'
        data4 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        msg5 = f'换卡请求的等待时间{POKE_DAILY_LIMIT}'
        data5 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        msg6 = f'每人每天最多接受几次赠卡{POKE_DAILY_LIMIT}'
        data6 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        msg7 = f'每日戳一戳、赠送等指令使用次数的重置时间{POKE_DAILY_LIMIT}:00'
        data7 = {
            "type": "node",
            "data": {
                "name": f"{forward_msg_name}",
        ...
        if recall_msg_set == 1:
            recall_1 = await bot.send(ev, poke_setup_help)
            notice = await bot.send(ev, f"将在{RECALL_MSG_TIME}s后将撤回消息")
            ...
            await bot.send(ev, poke_setup_help)

@sv.on_fullmatch(["帮助pcr戳一戳"])
async def bangzhu_poke(bot, ev):
    if forward_msg_exchange == 1:
        msg = sv_help
        data = {
            "type": "node",
